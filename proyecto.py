class Proyecto:
    ...
    def mostrar_resumen(self):
        print(f"[Codigo: {self.nombre_proyecto}] - {self.nombre_proyecto} | Presupuesto: ${self.presupuesto:,.0f}")

    def asociar_miembro(self, emp):
        self.equipo_asignado.append(emp)
        print(f" {emp.nombre_completo} asociado al priyecto {self.nombre_proyecto}")

    def remover_miembro(self, emp):
        if emp in self.equipo_asignado:
            self.equipo_asignado.remove(emp)
